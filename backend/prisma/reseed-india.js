/**
 * Safe Database Reseed Script
 * Deletes existing products and seeds with India-specific products
 * 
 * Run with: node reseed-india.js
 */
...
async function main() {
    console.log('🇮🇳 Starting safe database reseed with India products...\n');

    try {
        // Step 1: Delete existing products (CASCADE will delete variants too)
        console.log('🗑️  Deleting existing products...');
        const deleteResult = await prisma.product.deleteMany({});
        console.log(`✅ Deleted ${deleteResult.count} products\n`);

        // Step 2: Seed with India products
        ...
        for (const productData of indiaProducts) {
            ...
            createdProduct.variants.forEach(v => {
                console.log(`   - ${v.displayName}: ₹${v.retailPrice}`);
            });
            createdCount++;
        }

        console.log(`\n✨ Successfully seeded ${createdCount} India-specific products!\n`);
        console.log('🎉 Database reseed complete! All products now have realistic India pricing.\n');
